...
def main():
    proofingTime = BreadPredictor(bowlHeight=70)
    proofingTime.ingredWeight()
    distance = 30
    while True:
        #dataPacket= #receive data packet 
        proofingTime.insertData(distance,30,60)
        timeRemaining = proofingTime.predictTime()#predict time remaining
        print('mins left', timeRemaining)
        print('ingredientTime', proofingTime.ingredientTime)
        print(proofingTime.tempWeight(), proofingTime.humidWeight(), proofingTime.heightWeight())
        distance -=0.01
        time.sleep(1)
